...
// LIFF-based approval request: creates a Flex with a LIFF deep link to confirm/approve
export function bookingApprovalRequest(booking) {
  ...
  if (confirmLiffId && bookingId) {
    // LIFF deep link: the LiffQueryRouter will parse bookingId from query and route to /confirm/booking/:id
    uri = `https://liff.line.me/${confirmLiffId}?bookingId=${encodeURIComponent(bookingId)}`;
  }

  const actions = uri && /^https?:\/\//i.test(uri) ? { uri, label: 'ไปยังหน้าตรวจสอบ' } : null;
  return { altText: `คำขออนุมัติการจอง: ${booking.requesterName || ''}`, contents: baseBubble(title, fields, '', actions) };
}
